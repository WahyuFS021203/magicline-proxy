export default async function handler(req, res) {
  ...
  if (!queryParams) {
    return res.status(400).json({
      ...
      message:
        "Minimal berikan satu parameter pencarian (misal: customerNumber atau cardNumber)",
    });
  }
  ...
  try {
    ...
    // 5. Parsing Response dengan aman
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        ...
        message:
          data.message ||
          data.errorMessage ||
          "Gagal mencari data pelanggan dari Magicline",
      });
    }
