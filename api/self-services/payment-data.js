export default async function handler(req, res) {
  ...
  try {
    console.log(
      `[API Proxy] Mengambil data pembayaran untuk Customer ID: ${customerId}`,
    );
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        ...
        message:
          data.message ||
          data.errorMessage ||
          "Gagal mengambil data pembayaran dari Magicline",
      });
    }

    console.log(`[API Proxy] Data pembayaran berhasil diambil!`);
