export default async function handler(req, res) {
  ...
  const url = `${baseUrl}/v1/customers/${customerId}/self-service/contact-data`;
  ...
  try {
    ...
    // ==========================================
    // JIKA GET (MENGAMBIL DATA KONTAK)
    // ==========================================
    if (req.method === "GET") {
      console.log(
        `[API Proxy] GET: Fetching contact data for ID: ${customerId}`,
      );
    ...
    // ==========================================
    // JIKA POST (MEMPERBARUI DATA KONTAK)
    // ==========================================
    else if (req.method === "POST") {
      console.log(
        `[API Proxy] POST: Updating contact data for ID: ${customerId}`,
      );
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        ...
        message:
          data.message || data.errorMessage || "Gagal memproses data kontak",
      });
    }
